...
export async function* fetchDifyStreamResultW(
  messages: DifyMessage[]
): AsyncGenerator<{ word: string; thinking: boolean }> {
...
export async function* fetchDifyStreamResultAgent(
  messages: DifyMessage[],
  conversation_id: string
): AsyncGenerator<{ word: string; thinking: boolean }> {
  const baseUrl = process.env.NEXT_PUBLIC_DIFY_URL;
  ...
  const token = process.env.NEXT_PUBLIC_DIFY_TOKEN_forchat;
  if (!token) {
    throw new Error("DIFY_TOKEN未在环境变量中定义");
  }
  const options: RequestInit = {
